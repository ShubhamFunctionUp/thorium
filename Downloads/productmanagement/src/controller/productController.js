...
const CreateProduct = async function (req, res){
    try{
        ...
        if(!isValidCurrencyFormat(currencyFormat)){
            return res.status(400).send({status:false,message:"currencyFormat should be valid"})
        }

        if(!isValid(installments)){
            return res.status(400).send({status:false,message:"installments should be in Number only"})
        }
    ...
    try {
        ...
        if(priceLessThan){
            if(!validator.isValid(priceLessThan)){
                return res.status(400).send({status:false,message:"Please enter the pricelessthan"})
            }else{
...
let getProductById = async function(req,res){
    ...
    try{
        ...
        if(!isProductIdPresent){
            return res.status(400).send({status:false,message:"Product id is not present"})
        }
...
const updateProductById = async function (req, res) {
    try {
        ...
        if(!productId){
            return res.status(400).send({
                status: false,
                message: "productId is must please provide productId ",
              });
        }
        ...
        if (availableSizes) {
            ...
            if (availableSizes.length === 0) {
                return res.status(400).send({ status: false, message: "Input product size" })
            }
...
const deleteProduct = async function(req,res){
    try {
        ...
        if (!validator.isValidObjectId(productId)) {
           ...
           if(!isProductIdPresent){
            return res.status(400).send({status:false,message:"Product Id is already deleted"})
           }

           let nowDeleted = await ProductModel.findByIdAndUpdate(productId,{isDeleted:true},{new:true})

           return res.status(200).send({status:false,message:"success" ,data:nowDeleted})
